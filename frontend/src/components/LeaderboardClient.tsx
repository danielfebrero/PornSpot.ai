...
import type { LeaderboardUserEntry, UnifiedLeaderboardResponse } from "@/types";
import {
  useLeaderboard,
...
export function LeaderboardClient({ initialData }: LeaderboardClientProps) {
  ...
  // Fetch leaderboard data with infinite scroll, using initial data from ISR
  const {
    data,
    isLoading,
    error,
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useLeaderboard({ limit: 50, initialData });
  ...
  // Loading state (only for initial client-side load, not ISR)
