...
async function main() {
  ...
  let processed = 0; let updated = 0; let skipped = 0; let errors = 0;
  ...
  do {
    try {
      ...
      try {
        ...
        if (planFilter && plan !== planFilter) { skipped++; continue; }

        if (!shouldIndex(plan)) { skipped++; continue; }

        if (user.GSI4PK && user.GSI4SK) { skipped++; continue; }

        const { GSI4PK, GSI4SK } = buildGsi4(plan, planEndDate, userId);

        console.log(`User ${userId}: plan=${plan} end=${planEndDate || 'N/A'}`);
    ...
    console.log(`Batch complete. Processed=${processed} Updated=${updated} Skipped=${skipped} Errors=${errors}`);
