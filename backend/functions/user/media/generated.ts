...
const handleListUserGeneratedMedia = async (
  event: APIGatewayProxyEvent,
  auth: AuthResult
): Promise<APIGatewayProxyResult> => {
  const userId = auth.userId;
  console.log("✅ Authenticated user for generated media:", userId);
  ...
  // Get user's generated media by filtering for metadata.isGenerated = true
  const { media, nextKey } = await DynamoDBService.getUserGeneratedMedia(
    userId,
    limit,
    lastEvaluatedKey
  );
...
export const handler = LambdaHandlerUtil.withAuth(handleListUserGeneratedMedia);